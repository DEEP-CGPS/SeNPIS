...
fo=Fit(Net=net,trainloader=trainloader,testloader=testloader
                  ...
                  SaveDir="Default",Net_Name="Best-AlexNet-cifar10")
fo.DoTrain(Epoch=20)

net=torch.load("Default/Best-AlexNet-cifar10.pth")
...
torch.save(net,"Default/PrunedAlexNet-cifar10-P.pth")

#The pruned network is retrained "P+FT"
optimizer = optim.SGD(net.parameters(), lr=0.001, momentum=0.9)
fo=Fit(Net=net,trainloader=trainloader,testloader=testloader
                  ,valloader=testloader,device=device,NV=3,
                  criterion=criterion,optimizer=optimizer,Net_Name="PrunedAlexNet-cifar10-P+FT")
fo.DoTrain(Epoch=10)
